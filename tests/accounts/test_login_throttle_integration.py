...
def _login_with_csrf(client: APIClient, data: dict):
...
def test_invalid_login_increments_cached_attempt_counter():
    ...
    assert cache.get("login_attempts:missing-user") == 1
...
def test_successful_login_resets_cached_attempt_counter():
    ...
    assert cache.get(f"login_attempts:{user.username}") == 2
    ...
    assert cache.get(f"login_attempts:{user.username}") is None
...
def test_login_is_allowed_again_after_cached_block_expires(monkeypatch):
    ...
    assert cache.get("login_attempts:temporary-block-user") == 1
