...
def test_send_email_view_creates_email_log_and_sends_mail(regular_client, regular_user):
    ...
    assert response.data["created_by"] == regular_user.username
    assert response.data["metadata"]["sender_source"] == "default"
    assert EmailLog.objects.count() == 1
    assert len(mail.outbox) == 1


def test_send_email_view_accepts_attachment(regular_client):
